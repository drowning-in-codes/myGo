...
func main() {
	...
	if value.Kind() == reflect.Array || value.Kind() == reflect.Slice {
		...
		for i := 0; i < value.Len(); i++ {
			...
			allow_img_site = append(allow_img_site, value.Index(i).String())
		}
	...
	c.OnHTML("div[class='_article-main']", func(e *colly.HTMLElement) {
		...
		err := os.Mkdir(filepath.Join(download_root_folder, title), os.ModePerm)
		if err != nil {
			log.Default().Println(err.Error())
		}
		e.ForEach("div.article-item:not(._feature-article-body__paragraph)", func(i int, h *colly.HTMLElement) {
